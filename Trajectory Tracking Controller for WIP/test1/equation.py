...
class WIP_LINEAR(Equation):
    ...
    def b0_t(self, length, t):
        b0t = torch.zeros([length, self.dim, 1])
        b0_t = torch.zeros((4, 1))
        b0_t[0] = (80.340 * t ** 2 - 2134.4 * t + 4996.8) * np.exp(-0.5 * t)
        b0_t[1] = (63.476 * t ** 2 - 1638.4 * t + 3771.5) * np.exp(-0.5 * t)
        b0_t[2] = (10.578 * t ** 2 - 284.25 * t + 758.47) * np.exp(-0.5 * t)
        b0_t[3] = (60.605 * t ** 2 - 1564.6 * t + 3604.3) * np.exp(-0.5 * t)
        for i in range(length):
            b0t[i] = b0_t
        return b0t

    def P0_t(self, length):
        R = np.array([self.R])
        P0 = Liccati(self.A.numpy(), self.G.numpy(), self.Q.numpy(), R)
        P0 = torch.from_numpy(P0)
        P0_ = torch.zeros([length, self.dim, self.dim])
        P0_[:, ] = P0
        return P0_
    ...
    def u(self, x, t):
        tmp = self.b0_t(x.shape[0], t) + torch.bmm(self.P0_t(x.shape[0]), x)
        u = -(1 / self.R) * torch.bmm(torch.transpose(self.G_(x.shape[0]), 1, 2), tmp)
        return u
