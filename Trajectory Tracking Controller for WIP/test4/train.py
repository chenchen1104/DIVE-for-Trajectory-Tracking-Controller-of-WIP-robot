...
def plot_result(x_list, dot_x_list, theta_list, dot_theta_list, x_desire_list, dot_x_desire_list, theta_desire_list,
                dot_theta_desire_list,
                delta_z_list, figure_number, n):
    '''绘图参数定义'''
    label = ["LSTM-based(unconstrained)", "LSTM-based(constrained)", "FC-based(unconstrained)", "FC-based(constrained)"]
    color = ["r", "g", "b", "y"]
    '''绘制x曲线'''
    ...
    plt.ylabel("x $(Degree)$", fontproperties='Times New Roman', fontsize=fontsize)
    for i in range(figure_number):
        plt.plot(x_list[i], label=label[i], color=color[i], linestyle="-", linewidth=linewidth)
    ...
    for i in range(figure_number):
        plt.plot(theta_list[i], label=label[i], color=color[i], linestyle="-", linewidth=linewidth)
    ...
    plt.yticks(fontproperties='Times New Roman', fontsize=fontsize)
    plt.xlabel("Time$(0.02s)$", fontproperties='Times New Roman', fontsize=fontsize)
    plt.ylabel("dot_x $(Degree/s)$", fontproperties='Times New Roman', fontsize=fontsize)
    for i in range(figure_number):
        plt.plot(dot_x_list[i], label=label[i], color=color[i], linestyle="-", linewidth=linewidth)
    plt.plot(dot_x_desire_list[0], label="$dotx_{target}$", linestyle="--", linewidth=linewidth)
    plt.legend(loc='best', prop={'family': 'Times New Roman', 'size': legend_font_size})
    ...
    for i in range(figure_number):
        plt.plot(dot_theta_list[i], label=label[i], color=color[i], linestyle="-", linewidth=linewidth)
    plt.plot(dot_theta_desire_list[0], label="$dottheta_{target}$", linestyle="--", linewidth=linewidth)
    plt.legend(loc='best', prop={'family': 'Times New Roman', 'size': legend_font_size})
    ...
    plt.yticks(fontproperties='Times New Roman', fontsize=fontsize)
    plt.xlabel("Time$(0.02s)$", fontproperties='Times New Roman', fontsize=fontsize)
    plt.ylabel("control $(Degree)$", fontproperties='Times New Roman', fontsize=fontsize)
    for i in range(figure_number):
        plt.plot(delta_z_list[i], label=label[i], color=color[i], linestyle="-", linewidth=linewidth)
    plt.legend(loc='best', prop={'family': 'Times New Roman', 'size': legend_font_size})
...
def plot(cfg, fbsde, n):
    path = ["forward_constrained_LSTM.pth", "forward_unconstrained_LSTM.pth", "forward_constrained_FC.pth",
            "forward_unconstrained_FC.pth"]
    # path = ["forward_constrained_LSTM.pth", "forward_unconstrained_LSTM.pth", "forward_constrained_FC.pth",
    #         "forward_unconstrained_FC.pth", "back_constrained_LSTM.pth", "back_unconstrained_LSTM.pth"]
    x = []
    dot_x = []
    theta = []
    dot_theta = []
    ...
    for p in path:
        ...
        for i in range(len(x_sample)):
            t = i * 0.02
            x_desire_list.append(3 * t)
        x_desire.append(x_desire_list)

        dot_x_desire_list = []
        for i in range(len(x_sample)):
            dot_x_desire_list.append(3)
        dot_x_desire.append(dot_x_desire_list)

        theta_desire_list = []
        for i in range(len(x_sample)):
            t = i * 0.02
            theta_desire_list.append((0.05 * (20 * t - t ** 2)) * np.exp(-0.5 * t))
        theta_desire.append(theta_desire_list)

        dot_theta_desire_list = []
        for i in range(len(x_sample)):
            t = i * 0.02
            dot_theta_desire_list.append(
                0.05 * (20 - 2 * t) * np.exp(-0.5 * t) - 0.05 * 0.5 * (20 * t - t ** 2) * np.exp(-0.5 * t))
        dot_theta_desire.append(dot_theta_desire_list)

    plot_result(x, dot_x, theta, dot_theta, x_desire, dot_x_desire, theta_desire, dot_theta_desire, delta_z, len(x), n)
