...
class FeedForwardModel(nn.Module):
    ...
    def x_desired(self, length, t):
        ...
        X_bar[:, 0, 0] = 3 * t
        X_bar[:, 1, 0] = 3
        X_bar[:, 2, 0] = self.fbsde.k * (self.fbsde.alpha * t - t ** 2) * np.exp(-self.fbsde.beta * t)
        X_bar[:, 3, 0] = self.fbsde.k * (self.fbsde.alpha - 2 * t) * np.exp(
            -self.fbsde.beta * t) - self.fbsde.k * self.fbsde.beta * (self.fbsde.alpha * t - t ** 2) * np.exp(
            -self.fbsde.beta * t)
        return X_bar

    def forward(self, dw):
        num_sample = dw.shape[0]
        gamma = self.fbsde.gamma_(num_sample)
        G = self.fbsde.G_(num_sample)
        R = self.fbsde.R
        ...
        error_y = torch.zeros([num_sample, self._dim, 1])

        totalx = []
        totalu = []
        totaly = []
        deltay = torch.zeros([num_sample, 1, 1])
        ...
        if self._config.back == False:
            for t in range(0, self._num_time_interval):
                w = self.fbsde.w(num_sample, time_stamp[t])
                ...
                #     z = self._config.zmax * my_sig(z)

                # u = self.fbsde.u(error_x, time_stamp[t])
                u = (-1 / R) * torch.bmm(torch.transpose(gamma, 1, 2), z)
                if self._config.constrained == True:
                    u = torch.clamp(u, -self._config.umax, self._config.umax)
                totalu.append(u)

                i1 = self.fbsde.delta_t * self.fbsde.h_th(time_stamp[t], error_x, z, u)
                i2 = self.fbsde.delta_t * torch.bmm(torch.transpose(z, 1, 2), gamma) * u
                y = y - i1 + i2

                item1 = torch.bmm(self.fbsde.A_(num_sample), error_x) * self.fbsde.delta_t
                error_x = item1 + G * u * self.fbsde.delta_t + w * self.fbsde.delta_t

            yT = self.fbsde.g_th(self._total_time, error_x, u)
            loss = torch.sum(abs((y.squeeze(2) - yT.squeeze(2)))) + torch.sum(yT)
            ...
            y0 = yT - deltay
            loss = (torch.var(y0))
            return loss, y0, yT, totalx, totalu
